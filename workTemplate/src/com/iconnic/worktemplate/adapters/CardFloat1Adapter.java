...
public class CardFloat1Adapter extends RecyclerView.Adapter<CardFloat1Adapter.ViewHolder>{
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder{
    	CardView cv; ImageView pic; HeaderTextView text; IconnicTextView text2;
        
    	ViewHolder(View itemView,int viewtype){super(itemView);
            if(viewtype==TYPE_ITEM2){
            	cv = (CardView)itemView.findViewById(R.id.cardfloat2);
            	//cv.setMinimumWidth(width*gridnum); cv.setMinimumHeight(height);
            	}
            else{
            	cv = (CardView)itemView.findViewById(R.id.cardfloat1);
    ...
    public CardFloat1Adapter(List<String> pic, List<String> text, List<String> text2, int width,int height,String callingactivity,int gridnum, Context context)
    ...
    @Override public CardFloat1Adapter.ViewHolder onCreateViewHolder(ViewGroup parent,int viewType){
        switch(viewType){
        case TYPE_ITEM1: View zero = LayoutInflater.from(parent.getContext()).inflate(R.layout.cardfloat1, parent, false);
        		ViewHolder vhzero = new ViewHolder(zero,viewType);
        		return vhzero;
        case TYPE_ITEM2: View one = LayoutInflater.from(parent.getContext()).inflate(R.layout.cardfloat2, parent, false);
				ViewHolder vhone = new ViewHolder(one,viewType);
				return vhone;
        default:View def = LayoutInflater.from(parent.getContext()).inflate(R.layout.cardfloat1, parent, false);

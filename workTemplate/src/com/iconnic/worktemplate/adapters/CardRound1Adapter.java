...
public class CardRound1Adapter extends RecyclerView.Adapter<CardRound1Adapter.ViewHolder>{
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder{
    	CardView cv; ImageView pic; HeaderTextView text;
        
    	ViewHolder(View itemView,int viewtype){super(itemView);
            if(viewtype==TYPE_ITEM2){
            	cv = (CardView)itemView.findViewById(R.id.cardround2);
            	//cv.setMinimumWidth(width*gridnum); cv.setMinimumHeight(height);
            	}
            else{
            	cv = (CardView)itemView.findViewById(R.id.cardround1);
    ...
    public CardRound1Adapter(List<String> pic, List<String> text, int width,int height,String callingactivity,int gridnum, Context context)
    ...
    @Override public CardRound1Adapter.ViewHolder onCreateViewHolder(ViewGroup parent,int viewType){
        switch(viewType){
        case TYPE_ITEM1: View zero = LayoutInflater.from(parent.getContext()).inflate(R.layout.cardround1, parent, false);
        		ViewHolder vhzero = new ViewHolder(zero,viewType);
        		return vhzero;
        case TYPE_ITEM2: View one = LayoutInflater.from(parent.getContext()).inflate(R.layout.cardround2, parent, false);
				ViewHolder vhone = new ViewHolder(one,viewType);
				return vhone;
        default:View def = LayoutInflater.from(parent.getContext()).inflate(R.layout.cardround1, parent, false);

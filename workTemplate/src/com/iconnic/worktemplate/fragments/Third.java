...
import com.iconnic.worktemplate.adapters.CardFlat1Adapter;
...
public class Third extends Fragment{
	...
	public View onCreateView(LayoutInflater inflator,ViewGroup container,Bundle savedInstanceState){
		view = inflator.inflate(R.layout.third,container,false);
	...
	public void initRecycler(){	
		...
		fastScroller = (FastScroller)view.findViewById(R.id.fastscroll); //fast scroller
        ...
        adapter = new CardContacts1Adapter(pic,text,text,text,thewidth,theheight,"Third",gridnum,context); recycler.setAdapter(adapter);
        fastScroller.setRecyclerView(recycler); //fast scroll after adapter attached to recycler
	...
	private static void refreshRecyclerAll(){
		...
		adapter = new CardContacts1Adapter(pic,text,text,text,thewidth,theheight,"Third",gridnum,context);
	...
	private void fakeData(){		
		for(int i=0;i<MainActivity.temptext.length;i++){
			text.add("MR "+MainActivity.temptext[i]); pic.add(MainActivity.temppic[i]); 
		}
		for(int i=0;i<MainActivity.temptext.length;i++){
			text.add("MRS "+MainActivity.temptext[i]); pic.add(MainActivity.temppic[i]); 
		}
		for(int i=0;i<MainActivity.temptext.length;i++){
			text.add("CHILD "+MainActivity.temptext[i]); pic.add(MainActivity.temppic[i]); 
		}
	}
